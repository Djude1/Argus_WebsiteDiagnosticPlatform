...
# 音频播放队列和工作线程 - 使用优先级队列
_audio_queue = queue.PriorityQueue(maxsize=24)
...
async def _broadcast_audio_optimized(pcm_data: bytes):
    ...
    try:
        ...
        lead_ms = 80 if idle_sec > 3.0 else 40
        tail_ms = 30
...
def is_audio_playing_now() -> bool:
    """返回本地语音播放器是否正在播报（用于ASR防回声）。"""
    try:
        with _playing_lock:
            playing = _is_playing
        queued = (_audio_queue.qsize() > 0)
        return bool(playing or queued)
    except Exception:
        return False
