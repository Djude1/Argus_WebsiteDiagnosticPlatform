...
本 skill 是 Argus 前端的**唯一視覺與互動規範**。動手改任何畫面前先讀完「鐵則摘要」，再翻對應章節。本檔只談「設計怎麼做」；技術硬限制（單檔架構、build 工具等）以 [`frontend/AGENTS.md`](../../../frontend/AGENTS.md) 為準，兩者衝突時硬限制優先。
...
> 樣式一律寫進 `styles.css`、用 CSS 變數、BEM-like 命名（`.頁面-元素`），**禁止 inline style**（動態計算值除外）。詳見 `frontend/AGENTS.md`。
...
## 實作硬限制（來自 `frontend/AGENTS.md`，不可違反）
