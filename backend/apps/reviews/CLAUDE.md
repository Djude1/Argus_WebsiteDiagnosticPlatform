...
| Model | 重點 |
|---|---|
| `PlatformReview` | `OneToOne` user、rating 1-5、title/comment/display_name、published/hidden、experience_at；本人可編修或刪除 |
| `ReviewResponse` | 每則評論最多一則官方回覆；管理員可新增、更新或移除 |
| `ReviewRevision` | 儲存每次本人編修前的 rating/title/comment/display_name，只供內部稽核 |
...
- 公開作者名稱只使用使用者自填 `display_name`；留白顯示「匿名已驗證使用者」，不得由帳號姓名或 Email 推導。
- 使用者可修正 rating 與原文；每次 PATCH 前先建立 `ReviewRevision`。
- 管理員不得覆寫 rating、title、comment 或 display_name，只能管理 `ReviewResponse` 與 published/hidden。
