...
沒有單一規則檔能保證由所有 Agent 執行器自動載入。ZCode 每 session **只**自動載入全域 `~/.zcode/AGENTS.md` ＋ workspace 根 `AGENTS.md`（子目錄**不掃描**、`@import` **不展開**、`CLAUDE.md` 執行期**不載入**），模組規則靠根 `AGENTS.md` 的必讀閘門與 skill 觸發（description 前 250 字元每輪注入 context）。Codex／AGENTS 相容工具以根目錄 `AGENTS.md` 為入口，Claude Code 以根目錄 `CLAUDE.md` 為入口並採使用者層 → 專案層 → 子目錄層 → `CLAUDE.local.md` 的串接架構。較長的共用規則應集中在 `docs/` 作為單一事實來源，再由兩個入口共同連結（各執行器載入事實見 [`專案導覽.md`](../專案導覽.md) 第一節）。
...
| 任一份 skill（`.agents/skills/` 或 `.claude/skills/`） | 另一份副本必須**同次同步為逐字相同**（`diff` 驗證）；`專案導覽.md` 第二節「SKILL 索引」 |
