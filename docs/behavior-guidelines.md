...
與專案無關的個人化環境調整（如使用者全域 skill 安裝、本機工具設定、個人工作流偏好）**不寫入**專案記憶、log 或任何共用文件；確需留存時只記在個人本機層（`CLAUDE.local.md`／`Codex.local.md` 或 Agent 的使用者層 memory），並標明為個人環境事實，不與專案事實混雜。
