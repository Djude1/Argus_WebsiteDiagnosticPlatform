...
# Avoid permission issues when ultralytics tries writing under user roaming dirs.
os.environ.setdefault("YOLO_CONFIG_DIR", os.path.join(os.getcwd(), ".yolo_config"))
# 在其它 import 之后加：
...
# ---- Runtime knobs ----
CAM_PROCESS_EVERY_N = max(1, int(os.getenv("CAM_PROCESS_EVERY_N", "2")))
CAM_STREAM_EVERY_N = max(1, int(os.getenv("CAM_STREAM_EVERY_N", "2")))
CAM_JPEG_QUALITY = int(os.getenv("CAM_JPEG_QUALITY", "75"))
RECORDER_EVERY_N = max(1, int(os.getenv("RECORDER_EVERY_N", "2")))

# ---- ASR Providers ----
from openai_asr import OpenAIASRSession
from moonshine_asr import MoonshineASRSession

# 只读取一次 ASR_PROVIDER，默认使用 openai，避免被覆盖回 moonshine
...
def _pcm_gain(pcm: bytes, gain: float) -> bytes:
    if not pcm:
        return b""
...
def _resample_pcm16_mono(pcm: bytes, src_rate: int, dst_rate: int):
    if not pcm:
        return b"", None
    if audioop is not None:
        return audioop.ratecv(pcm, 2, 1, src_rate, dst_rate, None)

    src = np.frombuffer(pcm, dtype=np.int16).astype(np.float32)
    if src.size == 0:
        return b"", None
    src_idx = np.arange(src.size, dtype=np.float32)
    dst_len = max(1, int(round(src.size * float(dst_rate) / float(src_rate))))
    dst_idx = np.linspace(0, src.size - 1, dst_len, dtype=np.float32)
    dst = np.interp(dst_idx, src_idx, src)
    dst = np.clip(dst, -32768, 32767).astype(np.int16)
    return dst.tobytes(), None


def should_use_vision_context(user_text: str) -> bool:
    """仅在明确视觉问题时才附带图像，避免无谓消耗token。"""
    if os.getenv("OPENAI_VISION_ALWAYS_ON", "0") == "1":
        return True
...
from omni_client import stream_chat as omni_stream_chat, OmniStreamPiece
from asr_core import (
    ASRCallback,
    set_current_recognition,
    stop_current_recognition,
)
from audio_player import initialize_audio_system, play_voice_text, is_audio_playing_now

# ===== AI Provider 配置 =====
AI_PROVIDER = os.getenv("AI_PROVIDER", "openai").strip().lower()

# 根据 AI_PROVIDER 导入对应的客户端
if AI_PROVIDER == "gemini":
    try:
        from gemini_client import stream_chat as gemini_stream_chat, GeminiStreamPiece
        stream_chat = gemini_stream_chat
        print(f"[AI] 使用 Gemini 作为 AI 提供商 (密钥池: {__import__('gemini_client').get_key_pool().key_count} 个)")
    except ImportError as e:
        print(f"[AI] Gemini 导入失败: {e}，回退到 OpenAI")
        stream_chat = omni_stream_chat
else:
    stream_chat = omni_stream_chat
    print(f"[AI] 使用 OpenAI 作为 AI 提供商")
...
async def ui_broadcast_raw(msg: str):
    ...
    for k, ws in list(ui_clients.items()):
        try:
            await asyncio.wait_for(ws.send_text(msg), timeout=1.50)
        except Exception:
            dead.append(k)
    for k in dead:
...
async def full_system_reset(reason: str = ""):
    ...
    """
    # 1) 音频&AI
    await hard_reset_audio(reason or "full_system_reset", close_clients=True)
...
def parse_item_search_command(user_text: str) -> Optional[str]:
    text = (user_text or "").strip()
    if not text:
        return None
    if "找到了" in text or "拿到了" in text:
        return None

    patterns = [
        r"^\s*(?:请)?(?:帮我)?(?:找一下|找一找|找找|找)\s*(.+?)\s*(?:。|！|？|$)",
        r"^\s*(?:请)?(?:帮我)?(?:寻找|搜寻|搜索)\s*(.+?)\s*(?:。|！|？|$)",
        r"^\s*(?:请)?我(?:要|想)(?:找|寻找)\s*(.+?)\s*(?:。|！|？|$)",
        r"^\s*(?:请)?(?:帮我)?(?:识别一下|识别)\s*(.+?)\s*(?:。|！|？|$)",
    ]
    for pat in patterns:
        ...
        item = re.sub(r"^(一下|一个|一瓶|一包|一盒)\s*", "", item)
        item = re.sub(r"\s*(吧|呀|呢|啊|嘛)$", "", item).strip()
        if item:
            return item
    # 兜底：句子里包含“找XXX”时提取后半部分
    m2 = re.search(r"找\s*([^\s，。,！!？?]+)", text)
    if m2:
        item = m2.group(1).strip()
        item = re.sub(r"\s*(吧|呀|呢|啊|嘛)$", "", item)
        if item:
            return item
    return None
...
async def start_ai_with_text_custom(user_text: str):
    ...
    item_cn = parse_item_search_command(user_text)
    if item_cn:
...
async def start_ai_with_text(user_text: str):
    ...
    async def _runner():
        ...
        # 组装（默认仅文本；明确视觉问题时才附带图像）
        content_list = []
        use_vision = should_use_vision_context(user_text)
        if use_vision and last_frames:
            try:
            ...
            print("[AI] text-only mode (no image attached)", flush=True)
        ...
        try:
            async for piece in stream_chat(content_list, voice="alloy", audio_format="wav"):
                ...
                if piece.audio_b64:
                    ...
                    if pcm24:
                        # 24k → 8k
                        pcm8k, rate_state = _resample_pcm16_mono(pcm24, 24000, 8000)
                        pcm8k = _pcm_gain(pcm8k, 0.60)
                        if pcm8k:
...
async def ws_audio(ws: WebSocket):
    ...
    try:
        while True:
            ...
            if "text" in msg and msg["text"] is not None:
                ...
                if cmd == "START":
                    ...
                    if streaming and recognition is not None:
                        await ws.send_text("OK:ALREADY_STARTED")
                        continue
                    await stop_rec()
                    loop = asyncio.get_running_loop()
                    def post(coro):
                    ...
                    cb = ASRCallback(
                        ...
                        is_playing_now_fn=lambda: (is_playing_now() or is_audio_playing_now()),
                    ...
                    if ASR_PROVIDER == "moonshine":
                        recognition = MoonshineASRSession(
                            api_key="",
                            model=os.getenv("MOONSHINE_MODEL", "small"),
                            sample_rate=SAMPLE_RATE,
                            callback=cb,
                        )
                    else:
                        recognition = OpenAIASRSession(
                            api_key=API_KEY,
                            model=MODEL,
                            sample_rate=SAMPLE_RATE,
                            callback=cb,
            ...
            # 錄製下行聲音           
                ...
                if streaming and recognition:
                    try:
                        ...
                        if recv_chunks % 50 == 0:
                            print(
                                f"[AUDIO RX] chunks={recv_chunks}, bytes={recv_bytes}, "
                                f"avg={recv_bytes // recv_chunks}B",
                                flush=True,
                            )
                    except Exception:
            ...
            #     if streaming and recognition:
            #         try:
            #             if os.getenv("RECORDER_CAPTURE_UPLINK", "1") == "1":
            #                 try:
            #                     sync_recorder.record_audio(msg["bytes"], text="[ESP32 uplink]")
            #                 except Exception:
            #                     pass
...
async def ws_camera_esp(ws: WebSocket):
    ...
    last_out_jpeg: Optional[bytes] = None
    ...
    try:
        while True:
            ...
            if "bytes" in msg and msg["bytes"] is not None:
                ...
                # 【新增】录制原始帧（节流）
                if frame_counter % RECORDER_EVERY_N == 0:
                    try:
                ...
                # 仅在需要处理时才解码，降低 CPU 压力
                should_process = (frame_counter % CAM_PROCESS_EVERY_N == 0)
                bgr = None
                if should_process or yolomedia_running or (orchestrator and orchestrator.get_state() == "TRAFFIC_LIGHT_DETECTION"):
                    try:
                ...
                if orchestrator and not yolomedia_running and bgr is not None:
                    current_state = orchestrator.get_state()
                    if not should_process and current_state != "TRAFFIC_LIGHT_DETECTION":
                        # 未到处理帧，直接用上次结果或原图回传
                        if camera_viewers and frame_counter % CAM_STREAM_EVERY_N == 0:
                            jpeg_data = last_out_jpeg if last_out_jpeg else data
                            dead = []
                    ...
                    if camera_viewers and out_img is not None:
                        ok, enc = cv2.imencode(".jpg", out_img, [int(cv2.IMWRITE_JPEG_QUALITY), CAM_JPEG_QUALITY])
                        if ok:
                            last_out_jpeg = enc.tobytes()
                            if frame_counter % CAM_STREAM_EVERY_N == 0:
                                dead = []
                                for viewer_ws in list(camera_viewers):
                                    try:
                                        await viewer_ws.send_bytes(last_out_jpeg)
                                    except Exception:
                                        dead.append(viewer_ws)
                                for d in dead:
                                    camera_viewers.discard(d)
...
async def imu_broadcast(msg: str):
    ...
    for ws in list(imu_ws_clients):
        try:
            await asyncio.wait_for(ws.send_text(msg), timeout=0.50)
        except Exception:
            dead.append(ws)
    for ws in dead:
        imu_ws_clients.discard(ws)
...
async def on_shutdown():
    ...
    # 停止音频和AI任务
    await hard_reset_audio("shutdown", close_clients=True)
