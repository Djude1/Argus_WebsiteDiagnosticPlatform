...
掃描範圍：`backend/apps/*/models.py` 全部 model（2026-09-19 查證）。9 個 Django app 中 7 個擁有 model，共 **32 張資料表**（`insights`、`agent` 無自有 model；`agent` 的執行紀錄放在 `scans` 的 AgentSession／AgentStep；2026-09-19 新增 LoginEvent／SubscriptionPlan／UserSubscription）。
