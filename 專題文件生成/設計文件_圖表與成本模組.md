...
掃描範圍：`backend/apps/*/models.py` 全部 model（2026-09-19 查證，以已提交版本為準）。9 個 Django app 中 7 個擁有 model，共 **29 張資料表**（`insights`、`agent` 無自有 model；`agent` 的執行紀錄放在 `scans` 的 AgentSession／AgentStep）。
...
| billing | CoinTransaction | 10 種 kind、amount、balance_after、scan_job／plan／site_rebuild／admin_actor FK | 不可竄改帳務 |
